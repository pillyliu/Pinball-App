...
- Recast the shared Library runtime contract around CAF-published data on both platforms:
  - `opdb_export.json`
...
- Removed runtime dependence on merged bridge payloads, slug-based rulesheet/game-info fallback behavior, and starter-pack naming assumptions.
...
- Android Library route-missing recovery now also uses the shared `AppPanelEmptyCard` seam for its message state instead of one last local `EmptyLabel` pocket.
