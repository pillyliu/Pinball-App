...
- GameRoom `Areas` now uses a single `Save` action on both platforms, with row selection loading the current area into the draft instead of exposing a separate `Edit` button.
- Android GameRoom now uses connected segmented controls without the Material checkmark for both the machine `Summary / Input / Log` selector and the home `Cards / List` layout selector, bringing those controls closer to the iOS selector pattern.
...
- iOS GameRoom service-entry, play-count, issue-entry, resolve-issue, ownership, media-entry, attachment-preview, media-edit, part/mod, and log-edit sheets now also route through the shared `AppToolbarCancelAction`, `AppToolbarConfirmAction`, and `AppToolbarDoneAction` seams in `AppToolbarActions.swift`, removing another visible pocket of raw feature-local toolbar buttons from GameRoom presentation flows.
...
- Android GameRoom import-review and settings-route headings now also use shared `AppCardTitle` and `AppCardSubheading` seams instead of the remaining local semibold text blocks in import review and settings shell cards.
- Android GameRoom selected-log-entry presentation now also uses the shared `AppCardSubheading` seam for its remaining local mini-headings, keeping the machine-detail presentation hierarchy aligned with iOS.
